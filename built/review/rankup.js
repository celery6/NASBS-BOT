...
import Submission from '../base/Submission.js';
...
async function doRankup(member, emoji, name, msg, roleId, i) {
    ...
    const embed = new Discord.MessageEmbed()
        .setTitle(`NEW RANK ACHIEVED! You're now a ${emoji} ${emoji} **${name}!** ${emoji} ${emoji}`)
        .setDescription(msg);
...
async function rankup(member, points, guild, i) {
    console.log('doing rankup');
    if (points >= guild.rank2.points &&
        points < guild.rank3.points &&
        !member.roles.cache.get(guild.rank2.id)) {
        return doRankup(member, guild.emoji, guild.rank2.name, `__As a ${guild.rank2.name}, you are now qualified to build **Medium Builds!**__\n\nExamples: Department stores, strip malls, parking garages, marinas, schools, mid-rise apartments, small airports/harbors, etc!`, guild.rank2.id, i);
    }
    else if (points >= guild.rank3.points &&
        ...
        if (userPoints[0].pointsTotal >= 100) {
            return doRankup(member, guild.emoji, guild.rank3.name, `__As a ${guild.rank3.name}, you are now qualified to build **Large Builds!**__\n\nExamples: Skyscrapers, high-rises, convention centers, universities, large airports/harbours, etc!`, guild.rank3.id, i);
        }
    }
    else if (points >= guild.rank4.points &&
        ...
        if (userPoints[0].pointsTotal >= 200) {
            return doRankup(member, guild.emoji, guild.rank4.name, `__As a ${guild.rank4.name}, you are now qualified to build **Monumental Builds!**__\n\nExamples: Stadiums, amusement parks, megamalls, large medical or educational complexes, etc!`, guild.rank4.id, i);
        }
    }
    else if (points >= guild.rank5.points &&
        !member.roles.cache.get(guild.rank5.id)) {
        const userPoints = await Submission.aggregate([
            {
                $match: {
                    userId: member.id,
        ...
        if (userPoints[0].pointsTotal >= 400) {
            return doRankup(member, guild.emoji, guild.rank5.name, `As a ${guild.rank5.name}, you win ***extreme*** **bragging rights** because wtf how did you build so much <:what:743604228299292722>.`, guild.rank5.id, i);
        }
    }
}
export { rankup };
