...
class Die extends Command {
    ...
    async run(i) {
        ...
        await Submission.updateOne({ _id: submissionId, guildId: i.guild.id }, { pointsTotal: pointsTotal, complexity: complexity }).lean();
        // update user doc pointstotal
        await User.updateOne({ id: userId, guildId: i.guild.id }, { $inc: { pointsTotal: increment } }, { upsert: true }).lean();
        i.reply(`ok added ${complexity} complexity to <@${userId}>'s build. [Link](${submissionMsg.url}). DIE!`);
    }
}
export { Die };
