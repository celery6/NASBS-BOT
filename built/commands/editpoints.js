...
class EditPoints extends Command {
    constructor(client) {
        super(client, {
            ...
            description: 'ATTENTION: USE /REVIEW COMMAND FOR REGULAR EDITING, CONSULT WITH OTHER ADMINS BEFORE USING!!!',
            reviewer: true,
            args: [
                {
                    name: 'submissionid',
    ...
    async run(i) {
        ...
        if (!isAccepted) {
            return i.reply('that one hasnt been graded yet <:bonk:720758421514878998>! `/review` it first');
        }
        ...
        await Submission.updateOne({ _id: submissionId, guildId: i.guild.id }, { $inc: { pointsTotal: amount } }).lean();
