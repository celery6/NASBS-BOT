...
import rankup from '../review/rankup.js';
...
class Review extends Command {
    ...
    async run(i) {
        ...
        async function review(reply, data, countType, countValue) {
            ...
            try {
                ...
                if (edit && originalSubmission) {
                    ...
                    const userId = submissionMsg.author.id;
                    await User.updateOne({ id: userId, guildId: i.guild.id }, {
                        $inc: {
                    ...
                    if (!userData || userData.dm == true) {
                        ...
                        await dm
                            ...
                            .catch((err) => {
                            console.log(err);
                            i.followUp(`${i.user} has dms turned off or something went wrong while sending the dm! ${err}`);
                        });
                    }
                    ...
                    await i.followUp(`SUCCESS YAY!!!<:HAOYEEEEEEEEEEAH:908834717913186414>\n\n<@${userId}> has ${reply}`);
                }
            }
            catch (err) {
                console.log(err);
        ...
        if (i.options.getSubcommand() == 'one') {
            ...
            return review(`gained **${pointsTotal} points!!!**\n\n*__Points breakdown:__*\nNumber of buildings (S/M/L): ${smallAmt}/${mediumAmt}/${largeAmt}\nQuality multiplier: x${quality}\nComplexity multiplier: x${complexity}\nBonuses: x${bonus}\n[Link](${submissionMsg.url})\n\n__Feedback:__ \`${feedback}\``, submissionData, 'buildingCount', smallAmt + mediumAmt + largeAmt);
        }
            ...
            return review(`gained **${pointsTotal} points!!!**\n\n*__Points breakdown:__*\nLand area: ${sqm} sqm\nQuality multiplier: x${quality}\nComplexity multiplier: x${complexity}\nBonuses: x${bonus}\nCollaborators: ${collaborators}\n[Link](${submissionMsg.url})\n\n__Feedback:__ \`${feedback}\``, submissionData, 'sqm', sqm);
        }
            ...
            return review(`gained **${pointsTotal} points!!!**\n\n*__Points breakdown:__*\nRoad type: ${roadType}\nQuality multiplier: x${quality}\nComplexity multiplier: x${complexity}\nDistance: ${roadKMs} km\nBonuses: x${bonus}\nCollaborators: ${collaborators}\n[Link](${submissionMsg.url})\n\nFeedback: \`${feedback}\``, submissionData, 'roadKMs', roadKMs);
        }
    }
}
export { Review };
