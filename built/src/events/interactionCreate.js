async function execute(client, interaction) {
    if ((!client.test && interaction.guild.id == '935926834019844097') ||
    ...
    if (!guildData)
        return interaction.reply('this server is not registered :(');
    ...
    try {
        if (command.reviewer == true) {
            ...
            if (!member.roles.cache.has(guildData.reviewerRole)) {
                return await interaction.reply('you do not have permission to use this command <:bonk:720758421514878998>');
            }
        }
