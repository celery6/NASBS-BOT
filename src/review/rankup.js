...
// same process for sending dm and upgrading role for all rankups
...
async function rankup(member, points, guild, i) {
  ...
  if (
    ...
    // check if builder has 50 pts of >1.5x quality size medium builds by summing points from ONE and MANY which meet that criteria
          ...
          pointsTotal: {
            $sum: {
              $cond: [
                ...
                { $multiply: [{ $multiply: ['$mediumAmt', 5] }, '$quality'] },
              ],
            },
          },
        },
    ...
    // check if builder has 150 pts of >1.5x quality size medium or large builds by summing points from ONE and MANY which meet that criteria
    const userPoints = await Submission.aggregate([
      {
        $match: {
          userId: member.id,
    ...
    if (userPoints[0].pointsTotal >= 400) {
      return doRankup(
        ...
        guild.emoji,
        guild.rank5.name,
        `__As a ${guild.rank5.name}, you win ***extreme*** **bragging rights** because wtf how did you build so much <:what:743604228299292722>.`,
        guild.rank5.id,
        i,
      )
    }
